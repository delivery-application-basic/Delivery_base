/**
 * DriverRegisterScreen - Backend: POST /auth/register/driver
 * Body: full_name, email (optional), phone_number, password, driver_license_number, id_card_number, vehicle_type
 */
...
export default function DriverRegisterScreen() {
  ...
  const [driver_license_number, setLicense] = useState('');
  ...
  const handleRegister = async () => {
    ...
    if (!validateRequired(driver_license_number, 'Driver license number').valid) errs.driver_license_number = 'Driver license number is required';
    if (!validateRequired(id_card_number, 'ID card number').valid) errs.id_card_number = 'ID card number is required';
    if (!validateRequired(vehicle_type, 'Vehicle type').valid) errs.vehicle_type = 'Vehicle type is required';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Input label="Driver license number" value={driver_license_number} onChangeText={setLicense} error={errors.driver_license_number} />
      <Input label="ID card number" value={id_card_number} onChangeText={setIdCard} error={errors.id_card_number} />
      <Input label="Vehicle type" value={vehicle_type} onChangeText={setVehicleType} error={errors.vehicle_type} placeholder="e.g. Bicycle, Motorcycle" />
